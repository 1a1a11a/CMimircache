...
int LRUTest(int argc, char* argv[]) {


    printf("test_begin!\n");

    if (argc < 5) {
        printf("usage: %s filename fileType dataType alg\n", argv[0]);
        exit(1);
    }
    reader_t* reader;
    ...
    if (argv[2][0] == 'c'){
        ...
        reader = setup_reader("../data/trace.vscsi", 'b', 'l', BLOCK_UNIT_SIZE, DISK_SECTOR_SIZE, init_params2);
    }
    else {
    ...
    double* hr = get_hit_ratio_seq(reader, -1, 0, -1);
    printf("hit rate p: %p\n", hr);

    guint64 *hc = get_hit_count_seq(reader, -1, 10, 20);
    printf("hit count p: %p\n", hc);
    ...
    get_hit_ratio_seq(reader, -1, 0, -1);
    get_miss_rate_seq(reader, -1, 0, -1);
    ...
    cal_save_reuse_dist(reader, "rd.save", NORMAL_REUSE_DISTANCE);
    load_reuse_dist(reader, "rd.save", NORMAL_REUSE_DISTANCE);
