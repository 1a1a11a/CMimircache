...
void heatmapTest() {
    ...
    reader_t* reader = setup_reader("../data/trace.csv", 'c', 'l', BLOCK_UNIT_SIZE, DISK_SECTOR_SIZE, csv_init_params);
    struct_cache* fifo = fifo_init(CACHESIZE, 'l', 0, NULL);
    ...
    struct_cache* optimal = optimal_init(CACHESIZE, 'l', BLOCK_UNIT_SIZE, (void*)opt_init_params);
    struct_cache* lru = LRU_init(CACHESIZE, 'l', BLOCK_UNIT_SIZE, NULL);
...
void new_test_func(){
    ...
    csvReader_init_params* csv_init_params = (void*) new_csvReader_init_params(5, -1, 2, -1, TRUE, ',', -1);
//    reader_t* reader = setup_reader("../data/trace.csv", 'c', 'l', BLOCK_UNIT_SIZE, DISK_SECTOR_SIZE, csv_init_params);
    reader_t* reader = setup_reader("../data/trace.txt", 'p', 'l', BLOCK_UNIT_SIZE, DISK_SECTOR_SIZE, NULL);
    struct optimal_init_params* opt_init_params = g_new0(struct optimal_init_params, 1);
    opt_init_params->next_access = NULL;
    opt_init_params->reader = reader;
    struct_cache* optimal = optimal_init(CACHESIZE, 'l', BLOCK_UNIT_SIZE, (void*)opt_init_params);
