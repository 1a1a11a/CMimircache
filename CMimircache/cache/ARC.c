...
void __ARC_insert_element(cache_t *cache, request_t *cp) {
...
gboolean ARC_check_element(cache_t *cache, request_t *cp) {
  /* check both segments */
  ARC_params_t *ARC_params = (ARC_params_t *)(cache->cache_params);
...
void __ARC_update_element(cache_t *cache, request_t *cp) {
  /* if in seg1, then move to seg2;
   * if in seg2, just update
   */
  ARC_params_t *ARC_params = (ARC_params_t *)(cache->cache_params);
  if (ARC_params->LRU1->core->check_element(ARC_params->LRU1, cp)) {
    ARC_params->LRU1->core->remove_element(ARC_params->LRU1, cp->label_ptr);
...
void __ARC_evict_element(cache_t *cache, request_t *cp) {
  ARC_params_t *ARC_params = (ARC_params_t *)(cache->cache_params);
  ...
  if (remove_ghost)
    remove_ghost->core->remove_element(remove_ghost, cp->label_ptr);
  (*size)--;
  old_cp_itemp = cp->label_ptr;
  cp->label_ptr = evicted_item;
#ifdef SANITY_CHECK
  ...
  cp->label_ptr = old_cp_itemp;

  g_free(evicted_item);
}

gpointer __ARC__evict_with_return(cache_t *cache, request_t *cp) {
  ...
  if (ARC_params->LRU1g->core->check_element(ARC_params->LRU1g, cp)) {
    ...
    // then add evicted to LRU2g
    evicted_item =
        ARC_params->LRU2->core->__evict_with_return(ARC_params->LRU2, cp);
    ARC_params->LRU1g->core->remove_element(ARC_params->LRU1g, cp->label_ptr);
    ARC_params->size2--;
    old_cp_itemp = cp->label_ptr;
    cp->label_ptr = evicted_item;
    ...
    // then add evicted to LRU1g
    evicted_item =
        ARC_params->LRU1->core->__evict_with_return(ARC_params->LRU1, cp);
    ARC_params->LRU2g->core->remove_element(ARC_params->LRU2g, cp->label_ptr);
    ARC_params->size1--;
    old_cp_itemp = cp->label_ptr;
    cp->label_ptr = evicted_item;
...
gboolean ARC_add_element(cache_t *cache, request_t *cp) {
  ARC_params_t *ARC_params = (ARC_params_t *)(cache->cache_params);
  gboolean retval;
    ...
    if ((ARC_params->size1 + ARC_params->size2 > cache->core->size) ||
        (ARC_params->LRU1->core->get_current_size(ARC_params->LRU1) +
         ARC_params->LRU2->core->get_current_size(ARC_params->LRU2)) >
            cache->core->size) {
      fprintf(stderr,
              "ERROR: in ARC add_element, after inserting, "
...
cache_t *ARC_init(guint64 size, char data_type, guint64 block_size,
                  void *params) {
  ...
  ARC_params->LRU1g = LRU_init(size * ARC_params->ghost_list_factor, data_type,
                               block_size, NULL);
  ARC_params->LRU2 = LRU_init(size, data_type, block_size, NULL);
  ARC_params->LRU2g = LRU_init(size * ARC_params->ghost_list_factor, data_type,
                               block_size, NULL);
