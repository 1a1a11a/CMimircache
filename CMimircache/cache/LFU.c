 ...
 *
 * this LFU clear the frequency of an label after evicting from cache
...
/********************************** LFU ************************************/
void __LFU_insert_element(cache_t *LFU, request_t *cp) {
  ...
  node->data_type = cp->label_type;
  node->item = (gpointer)key;

  // frequency not cleared after eviction
  //    gpointer gp = g_hash_table_lookup(LFU_params->hashtable, cp->label_ptr);
...
gboolean LFU_add_element(cache_t *cache, request_t *cp) {
  LFU_params_t *LFU_params = (LFU_params_t *)(cache->cache_params);
  gboolean retval;
...
gboolean LFU_add_element_only(cache_t *cache, request_t *cp) {
  return LFU_add_element(cache, cp);
}

gboolean LFU_add_element_with_size(cache_t *cache, request_t *cp) {
  int i, n = 0;
...
cache_t *LFU_init(guint64 size, char data_type, guint64 block_size,
                  void *params) {
  ...
  if (data_type == 'l') {
    LFU_params->hashtable = g_hash_table_new_full(g_int64_hash, g_int64_equal,
...
void LFU_remove_element(cache_t *cache, void *data_to_remove) {
  LFU_params_t *LFU_params = (LFU_params_t *)(cache->cache_params);
  pq_node_t *node = (pq_node_t *)g_hash_table_lookup(
