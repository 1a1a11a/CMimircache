...
void __FIFOSize_insert_element(cache_t *cache, request_t *cp) {
  ...
  g_hash_table_insert(FIFOSize_params->hashtable, cache_obj->key,
                      (gpointer)node);
}

gboolean FIFOSize_check_element(cache_t *cache, request_t *cp) {
  struct FIFOSize_params *FIFOSize_params =
      (struct FIFOSize_params *)(cache->cache_params);
  return g_hash_table_contains(FIFOSize_params->hashtable, cp->label_ptr);
}

void __FIFOSize_update_element(cache_t *cache, request_t *cp) {
  struct FIFOSize_params *FIFOSize_params =
      (struct FIFOSize_params *)(cache->cache_params);
  GList *node =
      (GList *)g_hash_table_lookup(FIFOSize_params->hashtable, cp->label_ptr);
...
void __FIFOSize_evict_element(cache_t *cache, request_t *cp) {
  struct FIFOSize_params *FIFOSize_params =
      (struct FIFOSize_params *)(cache->cache_params);
...
gpointer __FIFOSize__evict_with_return(cache_t *cache, request_t *cp) {
...
gboolean FIFOSize_add_element(cache_t *cache, request_t *cp) {
  struct FIFOSize_params *FIFOSize_params =
      (struct FIFOSize_params *)(cache->cache_params);
  if (cp->size == 0) {
...
cache_t *FIFOSize_init(guint64 size, char data_type, guint64 block_size,
                       void *params) {
...
void FIFOSize_remove_element(cache_t *cache, void *data_to_remove) {
  struct FIFOSize_params *FIFOSize_params =
      (struct FIFOSize_params *)(cache->cache_params);
  ...
  if (!node) {
    fprintf(stderr,
            "FIFOSize_remove_element: data to remove is not in the cache\n");
    abort();
  }
