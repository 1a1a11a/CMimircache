...
static inline void __Mithril_rec_min_support_one(cache_t *Mithril,
                                                 request_t *cp);
...
static inline gint __Mithril_get_total_num_of_ts(gint64 *row, gint row_length) {
  int i, t;
  int count = 0;
...
static inline gboolean __Mithril_remove_from_rmtable(char data_type,
  ...
  if (data_type == 'l') {
    ...
    fprintf(stderr,
            "does not recognize data label_type in __Mithril_record_entry\n");
  return FALSE;
}
 ...
 */
static inline gboolean __Mithril_check_sequential(cache_t *Mithril,
                                                  request_t *cp) {
  int i;
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  ...
  if (Mithril->core->data_type != 'l')
    ERROR("sequential prefetching only support data label_type l\n");

  gint64 last = *(gint64 *)(cp->label_ptr);
  gboolean is_sequential = TRUE;
  gpointer old_item_p = cp->label_ptr;
  cp->label_ptr = &last;
  ...
  for (i = 0; i < sequential_K; i++) {
    last--;
    if (!Mithril_params->cache->core->check_element(Mithril_params->cache,
                                                    cp)) {
      is_sequential = FALSE;
      break;
    }
  }
  cp->label_ptr = old_item_p;
  return is_sequential;
}

void __Mithril_insert_element(cache_t *Mithril, request_t *cp) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  Mithril_params->cache->core->__insert_element(Mithril_params->cache, cp);
}

static inline void __Mithril_record_entry(cache_t *Mithril, request_t *cp) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  rec_mining_t *rmtable = Mithril_params->rmtable;
  ...
  if (Mithril_params->sequential_type &&
      __Mithril_check_sequential(Mithril, cp))
    return;
  ...
  if (Mithril_params->min_support == 1) {
    ...
    // check the label in hashtable for training
    gint index =
        GPOINTER_TO_INT(g_hash_table_lookup(rmtable->hashtable, cp->label_ptr));
    ...
    if (index == 0) {
      ...
      if (cp->label_type == 'l') {
        ...
        row_in_rtable[0] = *(gint64 *)(cp->label_ptr);
        ...
        gchar *str = g_strdup(cp->label_ptr);
        row_in_rtable[0] = (gint64)str;
        g_hash_table_insert(rmtable->hashtable, str,
                            GINT_TO_POINTER(rmtable->rtable_cur_row));
      } else {
        ERROR("unsupport data label_type in __Mithril_record_entry\n");
      }
      ...
      if (row_in_rtable[0] != 0) {
        ...
        if (cp->label_type == 'c') {
          ...
          ERROR("unsupported data label_type\n");
          abort();
        }
      ...
      if (index < 0) {
...
#ifdef SANITY_CHECK
        if (cp->label_type == 'l') {
          if (*(gint64 *)(cp->label_ptr) != row_in_mtable[0]) {
            ERROR("inconsistent entry in mtable "
                  "and mining hashtable current request %ld, "
                  "mining table %ld\n",
                  *(gint64 *)(cp->label_ptr), row_in_mtable[0]);
            abort();
          }
        } else {
          if (strcmp(cp->label_ptr, (char *)(row_in_mtable[0])) != 0) {
            ERROR("inconsistent entry in mtable "
                  "and mining hashtable current request %ld, "
                  "mining table %s %s\n",
                  (char *)(cp->label_ptr), (char *)(row_in_mtable[0]));
            abort();
          }
        }
#endif
        ...
        if (timestamps_length == Mithril_params->max_support) {
          ...
          if (!__Mithril_remove_from_rmtable(cp->label_type, rmtable,
                                             row_in_mtable))
...
#ifdef SANITY_CHECK
        if (cp->label_type == 'l') {
          if (*(gint64 *)(cp->label_ptr) != row_in_rtable[0]) {
            ERROR("Hashtable recording found position not correct %ld %ld\n",
                  *(gint64 *)(cp->label_ptr), row_in_rtable[0]);
            abort();
          }
        } else {
          if (strcmp(cp->label_ptr, (char *)(row_in_rtable[0])) != 0) {
            ERROR("Hashtable recording found position not correct %s %s\n",
                  (char *)(cp->label_ptr), (char *)(row_in_rtable[0]));
            abort();
          }
        }
#endif
        ...
        if (timestamps_length == Mithril_params->min_support - 1) {
          ...
          if ((long)rmtable->mining_table->len >= Mithril_params->mtable_size) {
             ...
             * the hashtable key not reliable when label_type is l */
          ...
          if (cp->label_type == 'l') {
             ...
             */
#ifdef SANITY_CHECK
            if (*inserted_row_in_mtable != *(gint64 *)(cp->label_ptr)) {
              ERROR("current block %ld, moving mining row block %ld\n",
                    *(gint64 *)(cp->label_ptr), *inserted_row_in_mtable);
              abort();
            }
#endif
...
static inline void __Mithril_rec_min_support_one(cache_t *Mithril,
...
#ifdef TRACK_BLOCK
  if (*(gint64 *)(cp->label_ptr) == TRACK_BLOCK) {
    int old_pos =
        GPOINTER_TO_INT(g_hash_table_lookup(rmtable->hashtable, cp->label_ptr));
  ...
  if (index == 0) {
    ...
    gpointer hash_key;
    if (cp->label_type == 'c') {
      array_ele[0] = (gint64)g_strdup(cp->label_ptr);
      hash_key = (void *)(array_ele[0]);
    } else {
      array_ele[0] = *(gint64 *)(cp->label_ptr);
    ...
    if (cp->label_type == 'l') {
      if (*(gint64 *)(cp->label_ptr) != row_in_mtable[0]) {
        ERROR("ts %lu, hashtable mining found position not correct %ld %ld\n",
              cp->ts, *(gint64 *)(cp->label_ptr), row_in_mtable[0]);
        abort();
      }
    } else {
      if (strcmp(cp->label_ptr, (char *)(row_in_mtable[0])) != 0) {
        ERROR("ts %lu, hashtable mining found position not correct %s %s\n",
              cp->ts, (char *)(cp->label_ptr), (char *)(row_in_mtable[0]));
        abort();
      }
    }
#endif
...
static inline void __Mithril_prefetch(cache_t *Mithril, request_t *cp) {

  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  gint prefetch_table_index = GPOINTER_TO_INT(
      g_hash_table_lookup(Mithril_params->prefetch_hashtable, cp->label_ptr));
  ...
  if (prefetch_table_index) {
    gpointer old_cp_gp = cp->label_ptr;
    int i;
    for (i = 1; i < Mithril_params->pf_list_size + 1; i++) {
      ...
      if (Mithril_params->ptable_array[dim1][dim2 + i] == 0)
        break;
      if (cp->label_type == 'l')
        cp->label_ptr = &(Mithril_params->ptable_array[dim1][dim2 + i]);
      else
        cp->label_ptr = (char *)(Mithril_params->ptable_array[dim1][dim2 + i]);
      ...
      if (Mithril_params->output_statistics) {
        ...
        if (cp->label_type == 'l') {
          ...
          *(guint64 *)item_p = *(guint64 *)(cp->label_ptr);
        } else
          item_p = (gpointer)g_strdup((gchar *)(cp->label_ptr));

        g_hash_table_insert(Mithril_params->prefetched_hashtable_Mithril,
                            item_p, GINT_TO_POINTER(1));
      }
    }
    cp->label_ptr = old_cp_gp;
  }

  // prefetch sequential
  if (Mithril_params->sequential_type == 1 &&
      __Mithril_check_sequential(Mithril, cp)) {
    gpointer old_gp = cp->label_ptr;
    gint64 next = *(gint64 *)(cp->label_ptr) + 1;
    cp->label_ptr = &next;
...
gboolean Mithril_check_element(cache_t *Mithril, request_t *cp) {
  ...
  gboolean result =
      Mithril_params->cache->core->check_element(Mithril_params->cache, cp);
...
void __Mithril_update_element(cache_t *Mithril, request_t *cp) {
...
void __Mithril_evict_element(cache_t *Mithril, request_t *cp) {
  ...
  if (Mithril_params->output_statistics) {
    gpointer gp;
    gp = Mithril_params->cache->core->__evict_with_return(Mithril_params->cache,
                                                          cp);
...
gboolean Mithril_add_element(cache_t *Mithril, request_t *cp) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  gboolean retval;
  Mithril_params->ts++;
  ...
  if (Mithril_params->cache->core->type == e_AMP) {
    retval = Mithril_check_element(Mithril, cp);
...
gboolean Mithril_check_element_only(cache_t *Mithril, request_t *cp) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  return Mithril_params->cache->core->check_element(Mithril_params->cache, cp);
}

void __Mithril_evict_element_only(cache_t *Mithril, request_t *cp) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
...
gboolean Mithril_add_element_only(cache_t *Mithril, request_t *cp) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  gboolean retval;

  if (Mithril_params->cache->core->type == e_AMP) {
    retval = Mithril_check_element_only(Mithril, cp);
...
gboolean Mithril_add_element_withsize(cache_t *Mithril, request_t *cp) {
  int i;
...
cache_t *Mithril_init(guint64 size, char data_type, guint64 block_size,
                      void *params) {
#ifdef SANITY_CHECK
  ...
  if (Mithril_params->max_support != 1) {
    rmtable->n_rows_in_rtable = (gint)(
        size * Mithril_params->block_size * RECORDING_TABLE_MAXIMAL /
        ((gint)ceil((double)Mithril_params->min_support / (double)2) * 2 + 8 +
         4));
  ...
  if (strcmp(init_params->cache_type, "LRU") == 0)
    ...
    Mithril_params->cache =
        AMP_init(size, data_type, block_size, AMP_init_params);
  ...
  if (data_type == 'l') {
    rmtable->hashtable =
        g_hash_table_new_full(g_int64_hash, g_int_equal, NULL, NULL);
    Mithril_params->prefetch_hashtable =
        g_hash_table_new_full(g_int64_hash, g_int64_equal, NULL, NULL);

    if (Mithril_params->output_statistics) {
      Mithril_params->prefetched_hashtable_Mithril = g_hash_table_new_full(
          g_int64_hash, g_int64_equal, simple_g_key_value_destroyer, NULL);
      Mithril_params->prefetched_hashtable_sequential = g_hash_table_new_full(
          g_int64_hash, g_int64_equal, simple_g_key_value_destroyer, NULL);
    }
  } else if (data_type == 'c') {
    rmtable->hashtable = g_hash_table_new_full(
    ...
    g_error("does not support given data label_type: %c\n", data_type);
  }
...
void __Mithril_mining(cache_t *Mithril) {
  Mithril_params_t *Mithril_params =
      (Mithril_params_t *)(Mithril->cache_params);
  rec_mining_t *rmtable = Mithril_params->rmtable;
#ifdef PROFILING
  ...
  /* first remove all elements from hashtable, otherwise after sort, it will
   mess up for label_type l but we can't do this for dataType c, otherwise the
  ...
  for (i = 0; i < (long)rmtable->mining_table->len - 1; i++) {
    ...
    for (j = i + 1; j < (long)rmtable->mining_table->len; j++) {
      ...
      if (GET_NTH_TS(item2, 1) - GET_NTH_TS(item1, 1) >
        ...
        break;
      num_of_ts2 =
          __Mithril_get_total_num_of_ts(item2, rmtable->mtable_row_len);
  ...
  if (prefetch_table_index) {
    ...
    if (g_hash_table_contains(Mithril_params->prefetch_hashtable, gp1)) {
      ...
      printf("ERROR datatype %c, contains %ld %s, value %d, %d\n",
             Mithril->core->label_type, *(gint64 *)gp1, (char *)gp1,
...
void __Mithril_aging(cache_t *Mithril) { ; }
