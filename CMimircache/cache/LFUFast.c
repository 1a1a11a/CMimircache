 ...
 *
 * this LFU_fast clear the frequenct of an label after evicting from cache
...
/********************************** LFU_fast
 * ************************************/
gboolean __LFU_fast_verify(cache_t *LFU_fast) {

  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(LFU_fast->cache_params);
  ...
  if (g_hash_table_size(LFU_fast_params->hashtable) == current_size)
    ...
    ERROR("hashtable size %u, queue accu size %lu\n",
          g_hash_table_size(LFU_fast_params->hashtable),
          (unsigned long)current_size);
    return FALSE;
  }
}

void __LFU_fast_insert_element(cache_t *LFU_fast, request_t *cp) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(LFU_fast->cache_params);
  ...
  g_hash_table_insert(LFU_fast_params->hashtable, (gpointer)key,
                      (gpointer)list_node);
  ...
  bnode_data->main_list_node =
      g_queue_peek_head_link(LFU_fast_params->main_list);
}

gboolean LFU_fast_check_element(cache_t *cache, request_t *cp) {
  return g_hash_table_contains(
      ((LFU_fast_params_t *)(cache->cache_params))->hashtable,
      (gconstpointer)(cp->label_ptr));
}

void __LFU_fast_update_element(cache_t *cache, request_t *cp) {
  /* find the given bnode_data, remove from current list,
   * insert into next main node */
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);
  ...
  if (exist_next && mnode_next_data->freq == mnode_data->freq + 1) {
    ...
    // insert mnode
    g_queue_insert_after(LFU_fast_params->main_list, bnode_data->main_list_node,
                         new_mnode_data);
  }
  bnode_data->main_list_node = bnode_data->main_list_node->next;
}

void __LFU_fast_evict_element(cache_t *cache, request_t *cp) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);

  GList *mnode = g_queue_peek_head_link(LFU_fast_params->main_list);
  main_list_node_data_t *mnode_data =
      g_queue_peek_head(LFU_fast_params->main_list);
  ...
  g_hash_table_remove(LFU_fast_params->hashtable,
                      (gconstpointer)(bnode_data->key));

  g_free(bnode_data);
}

gpointer __LFU_fast__evict_with_return(cache_t *cache, request_t *cp) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);
...
gboolean LFU_fast_add_element(cache_t *cache, request_t *cp) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);
  gboolean retval;
...
gboolean LFU_fast_add_element_only(cache_t *cache, request_t *cp) {
  return LFU_fast_add_element(cache, cp);
}

gboolean LFU_fast_add_element_withsize(cache_t *cache, request_t *cp) {
  int i, n = 0;
...
void LFU_fast_destroy(cache_t *cache) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);
...
void LFU_fast_destroy_unique(cache_t *cache) {
...
cache_t *LFU_fast_init(guint64 size, char data_type, guint64 block_size,
                       void *params) {
...
guint64 LFU_fast_get_size(cache_t *cache) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);
  return (guint64)g_hash_table_size(LFU_fast_params->hashtable);
}

void LFU_fast_remove_element(cache_t *cache, void *data_to_remove) {
  LFU_fast_params_t *LFU_fast_params =
      (LFU_fast_params_t *)(cache->cache_params);
  GList *blist_node =
      g_hash_table_lookup(LFU_fast_params->hashtable, data_to_remove);
