...
/* need to add support for p and c label_type of data
...
void __LRU_K_insert_element(cache_t *LRU_K, request_t *cp) {
  ...
  g_hash_table_lookup_extended(LRU_K_params->ghost_hashtable,
                               (gconstpointer)(cp->label_ptr), &key,
                               (gpointer)&queue);
...
gboolean LRU_K_check_element(cache_t *cache, request_t *cp) {
  ...
  *ts = LRU_K_params->ts;
  ...
  if (g_hash_table_contains(LRU_K_params->cache_hashtable,
                            (gpointer)(cp->label_ptr)))
    return TRUE;
  else
    return FALSE;
}

void __LRU_K_update_element(cache_t *cache, request_t *cp) {
  /* needs to update pq */
  struct LRU_K_params *LRU_K_params =
      (struct LRU_K_params *)(cache->cache_params);
  ...
  pq_node_t *node = (pq_node_t *)g_hash_table_lookup(
      LRU_K_params->cache_hashtable, (gconstpointer)(cp->label_ptr));
...
void __LRU_K_evict_element(cache_t *LRU_K) {
  ...
  g_hash_table_remove(LRU_K_params->cache_hashtable,
                      (gconstpointer)(node->item));
}

gboolean LRU_K_add_element(cache_t *cache, request_t *cp) {
  struct LRU_K_params *LRU_K_params =
      (struct LRU_K_params *)(cache->cache_params);
...
cache_t *LRU_K_init(guint64 size, char data_type, guint64 block_size,
                    void *params) {
  ...
  if (data_type == 'l') {
    // don't use pqueue_node_destroyer here, because the label inside node is
    // going to be freed by ghost_hashtable key
    LRU_K_params->cache_hashtable = g_hash_table_new_full(
        g_int64_hash, g_int64_equal, NULL, simple_g_key_value_destroyer);
    LRU_K_params->ghost_hashtable =
        g_hash_table_new_full(g_int64_hash, g_int64_equal,
                              simple_g_key_value_destroyer, gqueue_destroyer);
  } else if (data_type == 'c') {
    // don't use pqueue_node_destroyer here, because the label inside node is
    // going to be freed by ghost_hashtable key
    LRU_K_params->cache_hashtable = g_hash_table_new_full(
        g_str_hash, g_str_equal, NULL, simple_g_key_value_destroyer);
    LRU_K_params->ghost_hashtable =
