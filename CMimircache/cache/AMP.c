...
#define GET_BLOCK(cp)                                                          \
  (((request_t *)(cp))->label_type == 'l'                                      \
       ? *(gint64 *)(((request_t *)(cp))->label_ptr)                           \
       : atoll(((request_t *)(cp))->label))
...
struct AMP_page *__AMP_insert_element_int(cache_t *AMP, gint64 block) {
  struct AMP_params *AMP_params = (struct AMP_params *)(AMP->cache_params);
...
void __AMP_insert_element(cache_t *AMP, request_t *cp) {
  gint64 block = GET_BLOCK(cp);
  __AMP_insert_element_int(AMP, block);
}

gboolean AMP_check_element_int(cache_t *cache, gint64 block) {
  struct AMP_params *AMP_params = (struct AMP_params *)(cache->cache_params);
  return g_hash_table_contains(AMP_params->hashtable, &block);
}

gboolean AMP_check_element(cache_t *cache, request_t *cp) {
  gint64 block = GET_BLOCK(cp);
  return AMP_check_element_int(cache, block);
}

struct AMP_page *__AMP_update_element_int(cache_t *cache, gint64 block) {
  struct AMP_params *AMP_params = (struct AMP_params *)(cache->cache_params);
...
void __AMP_update_element(cache_t *cache, request_t *cp) {
  gint64 block = GET_BLOCK(cp);
  __AMP_update_element_int(cache, block);
}

void __AMP_evict_element(cache_t *AMP, request_t *cp) {
  struct AMP_params *AMP_params = (struct AMP_params *)(AMP->cache_params);
...
void *__AMP__evict_with_return(cache_t *AMP, request_t *cp) {
...
gboolean AMP_add_element_no_eviction(cache_t *AMP, request_t *cp) {
  struct AMP_params *AMP_params = (struct AMP_params *)(AMP->cache_params);
  gint64 block;
  if (cp->label_type == 'l')
    block = *(gint64 *)(cp->label_ptr);
  else {
    block = atoll(cp->label);
  }
...
gboolean AMP_add_element_only_no_eviction(cache_t *AMP, request_t *cp) {
...
gboolean AMP_add_element_only(cache_t *cache, request_t *cp) {
  /* only add element, do not trigger other possible operation */

#ifdef TRACKED_BLOCK
  if (*(gint64 *)(cp->label_ptr) == TRACKED_BLOCK)
...
gboolean AMP_add_element_no_eviction_withsize(cache_t *cache, request_t *cp) {
  struct AMP_params *AMP_params = (struct AMP_params *)(cache->cache_params);
  gint64 block, i, n = 1;
  ...
  if (AMP_check_element_int(cache, block)) {
    ...
    if (cp->disk_sector_size != 0 && cache->core->block_size != 0) {
      gint64 old_block = (*(gint64 *)(cp->label_ptr));
      for (i = 0; i < n - 1; i++) {
        (*(gint64 *)(cp->label_ptr))++;
        AMP_add_element_only(cache, cp);
      }
      (*(gint64 *)(cp->label_ptr)) = old_block;
    }
    // end
  ...
  // cache miss, load from disk
    ...
    if (cp->disk_sector_size != 0 && cache->core->block_size != 0) {
      ...
      gint64 last_block = (*(gint64 *)(cp->label_ptr)) + n - 1;
      ...
      for (i = 0; i < n - 1; i++) {
        ...
        AMP_add_element_only(cache, cp);
        AMP_lookup(cache, (*(gint64 *)(cp->label_ptr)))->last_block_number =
            last_block;
      }
      *(gint64 *)(cp->label_ptr) -= (n - 1);

#ifdef SANITY_CHECK
      if (*(gint64 *)(cp->label_ptr) != block)
        ERROR("current block %ld, original %ld, n %d\n",
              *(gint64 *)(cp->label_ptr), block, n);
...
gboolean AMP_add_element_withsize(cache_t *cache, request_t *cp) {
#ifdef TRACKED_BLOCK
  // debug
  if (*(gint64 *)(cp->label_ptr) == TRACKED_BLOCK)
    printf("ts %lu, add %d\n", cp->ts, TRACKED_BLOCK);
...
gboolean AMP_add_element(cache_t *cache, request_t *cp) {
#ifdef TRACKED_BLOCK
  // debug
  if (*(gint64 *)(cp->label_ptr) == TRACKED_BLOCK)
    printf("ts %lu, add %d\n", cp->ts, TRACKED_BLOCK);
  ...
  struct AMP_params *AMP_params = (struct AMP_params *)(cache->cache_params);
  gboolean result = AMP_add_element_no_eviction(cache, cp);
  while ((long)g_hash_table_size(AMP_params->hashtable) > cache->core->size)
    __AMP_evict_element(cache, cp);
  cache->core->ts += 1;
  return result;
...
cache_t *AMP_init(guint64 size, char data_type, guint64 block_size,
                  void *params) {
