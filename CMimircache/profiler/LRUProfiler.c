...
guint64* get_hit_count_seq_shards(reader_t* reader,
    ...
    if (reader->base->total_num == -1)
        total_requests = get_num_of_req(reader);
    ...
    // create splay tree
    sTree* splay_tree = NULL;
    int num_reference_count = 0;
    int num_unique_references = 0;
    read_one_element(reader, cp);
    while (cp->valid){
        int mod_val = pow(2, 24);
        int mod_limit = mod_val * sample_ratio;
...
double* get_hit_rate_seq_shards(reader_t* reader,
    ...
    if (reader->base->total_num == -1)
        reader->base->total_num = get_num_of_req(reader) * sample_ratio;

    if (size == -1)
        size = reader->base->total_num/sample_ratio;
...
static inline sTree* process_one_element_shards(cache_line* cp,
    ...
    if (gp == NULL){
        ...
        *reuse_dist = -1;
        ...
        // once I retrieve it.
        *reuse_dist = node_value(newtree->right)/sample_ratio;
        *(guint64*)gp = ts;
        newtree = splay_delete(old_ts, newtree);
        newtree = insert(ts, newtree);
